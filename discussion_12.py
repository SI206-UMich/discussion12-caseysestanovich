...
#inserts the above into the columns of the table 
...
def job_and_hire_date(cur, conn):
    ...
    data = sorted(data)
    job_id = data[0][1]
    cur.execute('SELECT job_title from jobs where job_id = ?', (job_id,))
...
def problematic_salary(cur, conn):
    cur.execute('select Employees.first_name, Employees.last_name from Employees join jobs.jobs_id = Employees.job_id where Employees.salary < jobs.min_salary or Employees.salary > jobs.max_salary')
    x = cur.fetchall()
    return x
